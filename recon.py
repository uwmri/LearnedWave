...
recon_type = 'train'
if recon_type == 'l1w' or recon_type == 'sense':
    num_iter = 30
coil_compression = False
if coil_compression:
    num_coils = 12
# as of 06/25/2024, no wave is 5242 epoch 20, wave is 5784 epoch 16
Ntrial = 5242
Nepoch = 20
if Ntrial == 5784:
    scale_global = 0.6468
...
train_dir = rf'D:\SSD\Data\run_flownoise\{Ntrial}'
centersfile = f'centers_opt_3D_{num_views}_{Ntrial}_{Nepoch}_permute.txt'
scalerotfile = f'scale_rot_{Ntrial}_{Nepoch}_permute.txt'
kwfile = f'kw_{Ntrial}_{Nepoch}_permute.txt'
helix_calib = np.loadtxt(os.path.join(opt_dir, f'helix_calibrated_r{radius}c{helix_cycles}echo{echo_frac}_fov{fov}.txt'), delimiter=",")
...
scan_root = r'D:\SSD\Data\Scans_i\wave_flow_vol\v22zyardim_2024-07-06'
scan_dir = r'D:\SSD\Data\Scans_i\wave_flow_vol\v22zyardim_2024-07-06\02145_00016_nowave_flow\raw_data'
...
# run this code without CNN denoiser, stepsize=1/max_eig, many unrolls first, to get an estimated image and scale_im
...
RUN=1
if RUN>0:
    run0fname = 'recon_train6_denoiserFalse.h5'
...
if recon_type == 'train':
    kw_raw = np.loadtxt(os.path.join(train_dir, kwfile), delimiter=',')  # kzc, kyc
    kw_raw = kw_raw.astype(np.float32)
    nro = kw_raw.shape[0]
    kw = np.zeros((nro*2, num_views), dtype=np.float32)
    for v in range(num_views):
        kw[:, v] = np.interp(np.arange(nro * 2), np.linspace(0, nro * 2 - 1, nro), kw_raw[:, v])
    kw = kw.reshape((-1,))
    # kw = np.ones_like(kw)
    kw = sp.to_device(kw, sp.Device(0))
...
if recon_type == 'train':
    recondelayfile = os.path.join(scan_dir, f'recon_{recon_type}{num_unroll}_denoiser{if_denoiser}.h5')
else:
...
for enc in range(num_enc):
    ...
    if recon_type == 'train':
        ...
        if RUN==0 and enc==0:
            # max_eig of training images are ~5.7 too
            max_eig = sp.app.MaxEig(A.H * A, max_iter=20, dtype=np.complex64, device=sp.Device(0),
                                    show_pbar=True).run()
